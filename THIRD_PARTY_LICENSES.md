# XLiteOCR — Third-Party License Manifest

Authoritative: resolved from each package's `License-Expression` / classifier
metadata (not the lossy `License:` free-text field). All permissive or
commercial-safe weak-copyleft (MPL-2.0). No GPL/AGPL.

| Package                | Version     | License                                                      |
| ---------------------- | ----------- | ------------------------------------------------------------ |
| albucore               | 0.0.13      | MIT License                                                  |
| albumentations         | 1.4.10      | MIT License                                                  |
| annotated-types        | 0.7.0       | MIT License                                                  |
| anyio                  | 4.14.1      | MIT                                                          |
| astor                  | 0.8.1       | BSD License                                                  |
| beautifulsoup4         | 4.15.0      | MIT License                                                  |
| certifi                | 2026.6.17   | Mozilla Public License 2.0 (MPL 2.0)                         |
| cffi                   | 2.0.0       | MIT                                                          |
| charset-normalizer     | 3.4.7       | MIT                                                          |
| click                  | 8.4.2       | BSD-3-Clause                                                 |
| contourpy              | 1.3.3       | BSD License                                                  |
| cryptography           | 49.0.0      | Apache-2.0 OR BSD-3-Clause                                   |
| cycler                 | 0.12.1      | BSD License                                                  |
| Cython                 | 3.2.6       | Apache-2.0                                                   |
| decorator              | 5.3.1       | BSD-2-Clause                                                 |
| fastapi                | 0.115.6     | MIT License                                                  |
| fire                   | 0.7.1       | Apache-2.0                                                   |
| fonttools              | 4.63.0      | MIT                                                          |
| h11                    | 0.16.0      | MIT License                                                  |
| httpcore               | 1.0.9       | BSD-3-Clause                                                 |
| httpx                  | 0.28.1      | BSD License                                                  |
| idna                   | 3.18        | BSD-3-Clause                                                 |
| ImageIO                | 2.37.3      | BSD-2-Clause                                                 |
| iniconfig              | 2.3.0       | MIT                                                          |
| joblib                 | 1.5.3       | BSD-3-Clause                                                 |
| kiwisolver             | 1.5.0       | BSD License                                                  |
| lazy-loader            | 0.5         | BSD-3-Clause                                                 |
| lmdb                   | 2.2.1       | OLDAP-2.8                                                    |
| lxml                   | 6.1.1       | BSD-3-Clause                                                 |
| matplotlib             | 3.11.0      | Python Software Foundation License                           |
| narwhals               | 2.22.1      | MIT                                                          |
| networkx               | 3.6.1       | BSD-3-Clause                                                 |
| numpy                  | 1.26.4      | BSD License                                                  |
| opencv-contrib-python  | 4.11.0.86   | Apache Software License                                      |
| opencv-python          | 4.11.0.86   | Apache Software License                                      |
| opencv-python-headless | 4.11.0.86   | Apache Software License                                      |
| opt-einsum             | 3.3.0       | OSI Approved                                                 |
| packaging              | 26.2        | Apache-2.0 OR BSD-2-Clause                                   |
| paddleocr              | 2.10.0      | Apache License 2.0                                           |
| paddlepaddle           | 3.3.1       | Apache Software License                                      |
| pdfminer.six           | 20240706    | MIT License                                                  |
| pillow                 | 11.3.0      | CMU License (MIT-CMU)                                        |
| pip                    | 26.1.2      | MIT                                                          |
| pip-licenses           | 5.0.0       | MIT License                                                  |
| pluggy                 | 1.6.0       | MIT License                                                  |
| prettytable            | 3.18.0      | BSD-3-Clause                                                 |
| protobuf               | 7.35.1      | 3-Clause BSD License                                         |
| pyclipper              | 1.4.0       | OSI Approved                                                 |
| pycparser              | 3.0         | BSD-3-Clause                                                 |
| pydantic               | 2.13.4      | MIT                                                          |
| pydantic_core          | 2.46.4      | MIT                                                          |
| Pygments               | 2.20.0      | BSD-2-Clause                                                 |
| pyparsing              | 3.3.2       | MIT                                                          |
| pypdfium2              | 4.30.0      | (Apache-2.0 OR BSD-3-Clause) AND LicenseRef-PdfiumThirdParty |
| pytest                 | 9.1.1       | MIT                                                          |
| python-dateutil        | 2.9.0.post0 | BSD License                                                  |
| python-docx            | 1.2.0       | MIT License                                                  |
| python-multipart       | 0.0.20      | Apache-2.0                                                   |
| PyYAML                 | 6.0.3       | MIT License                                                  |
| RapidFuzz              | 3.14.5      | MIT                                                          |
| requests               | 2.34.2      | Apache Software License                                      |
| safetensors            | 0.8.0       | Apache Software License                                      |
| scikit-image           | 0.26.0      | BSD License                                                  |
| scikit-learn           | 1.9.0       | BSD-3-Clause                                                 |
| scipy                  | 1.17.1      | BSD License                                                  |
| setuptools             | 82.0.1      | MIT                                                          |
| shapely                | 2.1.2       | BSD License                                                  |
| six                    | 1.17.0      | MIT License                                                  |
| soupsieve              | 2.8.4       | MIT                                                          |
| starlette              | 0.41.3      | BSD License                                                  |
| termcolor              | 3.3.0       | MIT                                                          |
| threadpoolctl          | 3.6.0       | BSD License                                                  |
| tifffile               | 2026.3.3    | BSD-3-Clause                                                 |
| tomli                  | 2.4.1       | MIT                                                          |
| tqdm                   | 4.68.3      | MPL-2.0 AND MIT                                              |
| typing-inspection      | 0.4.2       | MIT                                                          |
| typing_extensions      | 4.15.0      | PSF-2.0                                                      |
| urllib3                | 2.7.0       | MIT                                                          |
| uvicorn                | 0.34.0      | BSD License                                                  |
| vtracer                | 0.6.11      | MIT                                                          |
| wcwidth                | 0.8.1       | MIT                                                          |
| wheel                  | 0.47.0      | MIT                                                          |

_Total: 82 packages._
